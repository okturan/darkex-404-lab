/**
 * Shared bootstrap for every release artifact: reads colors from the CSS
 * variables, the mark from the embedded template (or the URL in
 * config.logo.src), derives the favicon from it, seeds the grains and runs
 * the loop. Artifacts differ only in what they lay over the canvas.
 */
...
export async function startAssembly(config) {
  // The mark: the SVG embedded in the page, unless logo.src points at a
  // hosted copy — then the URL wins. Either way it is re-issued as a data
  // URI, which keeps sampling origin-clean (even for CDN sources) and lets
  // the favicon derive from the exact artwork in use.
  const embedded = () =>
  ...
  // density is the artifact's business.
